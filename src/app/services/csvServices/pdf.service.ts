...
export class CSVService {
  ...
  private waterMark = 'Upperlink.ng';
  constructor(rendererFactory: RendererFactory2) {
    this.renderer = rendererFactory.createRenderer(null, null);

  }

  downloadCsvFormatForReports(reports: Array<any>, fileName, callback) {
    const data = [];
  ...
  downloadCsvFormatForFinalReports(reports: Array<any>, fileName, callback) {
    const data = [];
    const name = `${fileName} as at ${new Date().toTimeString()}.csv`;
    reports.forEach((report, index) => {
      const _report = {
  ...
  private generateCsvDownloader(data, callback) {
